package com.geekbang.coupon.config.loadbalancer;
...
import static com.geekbang.coupon.constant.CommonConstant.TRAFFIC_VERSION;

/**
 * 金丝雀负载均衡器
 *
 * @author wuyuexiang
 * @date 2022年09月16日 20:11
 */
@Slf4j
public class CanaryLoadBalancer implements ReactorServiceInstanceLoadBalancer {
    ...
    Response<ServiceInstance> getInstanceResponse(List<ServiceInstance> instances, Request request) {
        ...
        List<ServiceInstance> canaryInstances = instances.stream()
                .filter(e -> StringUtils.equalsIgnoreCase(e.getMetadata().get(TRAFFIC_VERSION), trafficVersion))
                .collect(Collectors.toList());
